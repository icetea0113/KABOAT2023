import heapq
from autonomous.utils import stateNameToCoords, render_all
...
## -> 위의 함수에서 범위 밖에 있음에도 불구하고 graph 반영되는 현상 체크하기
...
def moveAndRescan(graph, queue, s_current, scan_range, k_m):
    path = [s_current]  # Initialize the path with the current state
    direction = []
    if(s_current == graph.goal):
        return path, 'goal', k_m
    while(s_current != graph.goal):
        s_last = s_current
        s_new = nextInShortestPath(graph, s_current)
        new_coords = stateNameToCoords(s_new)
        ...
        computeShortestPath(graph, queue, s_current, k_m)
        current_x_y = stateNameToCoords(s_last)
        next_x_y = stateNameToCoords(s_new)
        if  current_x_y[0]-next_x_y[0] == 1:
            direction.append(2)
    ...
    s_new = path[1]
    print(direction[1:])
    return direction[1:], s_new, k_m


def initDStarLite(graph, queue, s_start, s_goal, k_m):
    graph.graph[s_goal].rhs = 0.0
