...
class Autonomous(Node):
    def __init__(self):
        ...
        self.queue_publish = self.create_publisher(Int8MultiArray, "list_load", qos_profile)
    ...
    def gps_listener_callback(self):
        ...
        msg_x = Int64()
        msg_y = Int64()
        msg_x.data = self.e_index
        msg_y.data = self.n_index
        self.curr_x_publish.publish(msg_x)
    ...
    def dstarlite(self):
        ...
        if self.s_current != self.now_position:
            ...
            msg = Int8MultiArray()
            msg.data = self.path
            self.queue_publish.publish(msg)
        if self.s_new == 'goal':
            print('Goal Reached!')
            del self.s_goals[0]
            self.s_goal = self.s_goals[0]
