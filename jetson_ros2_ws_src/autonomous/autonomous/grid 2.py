    ...
    def generateGraphFromGrid(self):
        ...
        for i in range(len(self.cells)):
            ...
            for j in range(len(row)):
                ...
                # #대각선 이웃 추가 (대각선 이동 비용은 √2로 가정)
                # if i > 0:  # not top row
                #     if j > 0:  # not left column
                #         node.parents['x' + str(i - 1) + 'y' + str(j - 1)] = edge * math.sqrt(2)
                #         node.children['x' + str(i - 1) + 'y' + str(j - 1)] = edge * math.sqrt(2)
                #     if j + 1 < self.x_dim:  # not right column
                #         node.parents['x' + str(i - 1) + 'y' + str(j + 1)] = edge * math.sqrt(2)
                #         node.children['x' + str(i - 1) + 'y' + str(j + 1)] = edge * math.sqrt(2)
                # if i + 1 < self.y_dim:  # not bottom row
                #     if j > 0:  # not left column
