 ...
 public:
  ...
  private:
    ...
    void receiveNavSatFix(const sensor_msgs::msg::NavSatFix::SharedPtr navsat_msg)
    {
  	...
  	int lat_degs = navsat_msg->latitude;
  	...
  	int lon_degs = floor(navsat_msg->longitude);
  	double lon_mins = (navsat_msg->longitude - (double) lon_degs) * 60.0;
	lon_degs = fabs(lon_degs);
	lon_mins = fabs(lon_mins);
	...
	//double alt_geoid = 1.2;
  	...
  	uint8_t
      	len = sprintf(buf,
                    "$GPGGA,%02ld%02ld%02ld.%ld,%02d%04.4f,%c,%03d%07.4f,%c,%d,%d,0.63,%0.3f,M,1.2,M,0.0,0000,",
                    time.hours(),
                    time.minutes(),
                    time.seconds(),
                    deci_seconds,
